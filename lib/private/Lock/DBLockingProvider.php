...
class DBLockingProvider extends AbstractLockingProvider {
	...
	public function acquireLock(string $path, int $type, ?string $readablePath = null): void {
		...
		if ($type === self::LOCK_SHARED) {
			if (!$this->isLocallyLocked($path)) {
				...
				if ($result <= 0) {
					...
					$query->update('file_locks')
						->set('lock', $query->func()->add('lock', $query->createNamedParameter(1)))
						->set('ttl', $query->createNamedParameter($expire))
						->where($query->expr()->eq('key', $query->createNamedParameter($path)))
						->andWhere($query->expr()->gte('lock', $query->createNamedParameter(0)));
			...
			if ($result <= 0) {
				$query = $this->connection->getQueryBuilder();
				$query->update('file_locks')
					->set('lock', $query->createNamedParameter(-1))
					->set('ttl', $query->createNamedParameter($expire))
					->where($query->expr()->eq('key', $query->createNamedParameter($path)))
	...
	public function releaseLock(string $path, int $type): void {
		...
		if ($type === self::LOCK_EXCLUSIVE) {
			...
			$query->update('file_locks')
				...
				->where($query->expr()->eq('key', $query->createNamedParameter($path)))
				->andWhere($query->expr()->eq('lock', $query->createNamedParameter(-1)));
		...
		if ($targetType === self::LOCK_SHARED) {
			...
			$result = $qb->update('file_locks')
				...
				->where($qb->expr()->andX(
					...
					$qb->expr()->eq('lock', $qb->createNamedParameter(-1))
				))->executeStatement();
		} else {
	...
	public function releaseAll(): void {
		...
		$builder->update('file_locks')
			->set('lock', $builder->func()->subtract('lock', $builder->expr()->literal(1)))
			->where($builder->expr()->in('key', $builder->createParameter('chunk')))
			->andWhere($builder->expr()->gt('lock', new Literal(0)));

		foreach ($chunkedPaths as $chunk) {
			$builder->setParameter('chunk', $chunk, IQueryBuilder::PARAM_STR_ARRAY);
